...
model = U_Net(img_ch=config['in_channels'], output_ch=config['n_classes'])
...
# please enter the source data root, submission root and mask root
source_root = r""
submission_root = r""
mask_root = r""
...
for city in cities:
    state_dict = torch.load(f"./{city}/checkpoint.pt", map_location=device)
